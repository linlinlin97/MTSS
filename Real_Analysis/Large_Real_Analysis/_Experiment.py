...
import Agents_SemiBandit._agent_SemiBandit_MTB_sparse as _agent_SemiBandit_MTB_sparse
...
#        if Bandit_type in ["MNL", "Cascading", "Semi-Bandit"]:
        if self.sparse:
            self.names = ["Oracle TS", "P = 4", "P = 10", "P = 50"]
        elif not real:
    ...
    def run_one_seed_sparse_SemiBandit(self, seed):
        ...
        MTTS_agent_sparse_p0 = _agent_SemiBandit_MTB_sparse.MTB_agent(sigma_2 = sigma_2, L=L, L_tot=self.L_tot, T = T
                                                         , gamma_prior_mean = self.mu_gamma, gamma_prior_cov = self.Sigma_gamma
                                                         , sigma_1 = sigma_1
                                                         , K = K
                                                         , Xs = self.exp.env.Phi 
        ...
        MTTS_agent_sparse_p1 = _agent_SemiBandit_MTB_sparse.MTB_agent(sigma_2 = sigma_2, L=L, L_tot=self.L_tot, T = T
                                                         , gamma_prior_mean = self.mu_gamma, gamma_prior_cov = self.Sigma_gamma
                                                         , sigma_1 = sigma_1
                                                         , K = K
                                                         , Xs = self.exp.env.Phi 
        ...
        MTTS_agent_sparse_p2 = _agent_SemiBandit_MTB_sparse.MTB_agent(sigma_2 = sigma_2, L=L, L_tot=self.L_tot, T = T
                                                         , gamma_prior_mean = self.mu_gamma, gamma_prior_cov = self.Sigma_gamma
                                                         , sigma_1 = sigma_1
                                                         , K = K
                                                         , Xs = self.exp.env.Phi 
    ...
    def run_multiple_parallel(self, reps, init_seed = 0, parallel = 'parmap'):
        ...
        import ray
        if parallel == 'parmap':
            if self.sparse and self.Bandit_type == "Cascading":
